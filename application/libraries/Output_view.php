...
class Output_view
{
    ...
    public function output($template, $data = null)
    {
        ...
        var base = "' . base_url() . '";
        var segment = ["' . $this->CI->uri->segment(1) . '", "' . $this->CI->uri->segment(2) . '", "' . $this->CI->uri->segment(3) . '"];
        var ctn="' . base64_encode($this->CI->config->item('csrf_token_name')) . '",ccn="' . base64_encode($this->CI->config->item('csrf_cookie_name')) . '";
        </script>';
        echo $view[0], $script, $view[1];
