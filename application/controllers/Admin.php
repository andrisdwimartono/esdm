...
class Admin extends MX_Controller
{
    ...
    public function page_builder()
    {
        ...
        base_url('assets/js/builder.js?v=4')
        ];
    ...
    public function crud_builder()
    {
        ...
        $crud->fields('table_name', 'subject', 'title', 'action', 'breadcrumb', 'table_config');
        ...
        base_url('assets/plugins/highlightjs/highlight.pack.js'),
        base_url('assets/plugins/clipboard/dist/clipboard.min.js'),
        base_url('assets/js/builder.js?v4')
        ];
    ...
    public function get_list_fields($table)
    {
        ...
        $this->output->set_content_type('application/html')->set_output($viewList);
    }
...
return $crumbItem;
    ...
    {
        return site_url('admin/table' . '/' . $row->table_name . '__' . $primary_key);
    }
    ...
    {
        return $this->template_link('admin/table/' . $row->table_name . '__' . $row->id_table);
    }
    ...
    public function module_detail($path)
    {
        ...
        $template_data['crumb'] = [
        'Module Extensions' => 'admin/modules',
        'Module Detail' => '',
        ];
        $this->output_view->output($template, $template_data);
    }
    ...
    public function active_callback($value, $row)
    {
        ...
        return "<a href='".site_url('admin/manual_activate/'.$row->id.'/'.$value)."'>$val</a>";
    }
    ...
    public function menu_type()
    {
        if ($this->uri->segment(4) == 'delete') {
            ...
            redirect('admin/menu/'.$this->uri->segment(3));
        } else {
            $crud = new grocery_CRUD();
        ...
        foreach ($menus as $menu) {
            if ($parent_id == $menu['parent_id']) {
                ...
                <span class="dd-action">
                  <a href="'.site_url('admin/crud_menu/'.$type.'/edit/'.$menu['id_menu']).'" title="edit"><i class="fa fa-pencil"></i></a>
                  <a href="'.site_url('admin/crud_menu/'.$type.'/delete/'.$menu['id_menu']).'" title="Delete" class="delete-confirm"><i class="fa fa-trash"></i></a>
              </span></p>';
    ...
    public function database()
    {
        ...
        $template_data['judul'] = 'Database Manager';
        ...
        $this->output_view->set_wrapper('page', 'database', $data);
        $this->output_view->auth();
